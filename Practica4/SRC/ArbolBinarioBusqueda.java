...
public class ArbolBinarioBusqueda<T extends Comparable<T>>
    ...
    private void agrega(T elemento, Nodo<T> n) {
        if (elemento.compareTo(n.elemento) <= 0 ) {
            if (n.izquierdo == null) {
                n.izquierdo = nuevoNodo(elemento);
                n.izquierdo.padre = n;
                tamanio++;
                return;
            }
            agrega(elemento,n.izquierdo);
        } else {
            if(n.derecho == null) {
                n.derecho = nuevoNodo(elemento);
                n.derecho.padre = n;
                tamanio++;
                return;
            }
            agrega(elemento,n.derecho);
        }
    }
